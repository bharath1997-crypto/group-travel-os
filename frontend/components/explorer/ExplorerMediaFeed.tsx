  ...
  return (
    <div className="flex gap-3 overflow-hidden pb-1 pl-1 pr-1 pt-1">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="relative h-[280px] w-[158px] shrink-0 overflow-hidden rounded-2xl border border-[#1E293B] bg-[#1E293B]"
        >
          <div className="absolute inset-0 animate-pulse bg-gradient-to-b from-[#1E293B]/40 via-[#1E293B] to-[#1E293B]" />
          <div className="absolute bottom-3 left-3 right-3 h-12 rounded-lg bg-[#1E293B]/50" />
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="overflow-hidden rounded-2xl border border-[#1E293B] bg-[#1E293B] p-5"
        >
          <div className="mb-4 h-3 w-24 animate-pulse rounded-full bg-[#1E293B]/60" />
          <div className="mb-2 h-5 w-full animate-pulse rounded-md bg-[#1E293B]/35" />
          <div className="mb-2 h-5 w-4/5 animate-pulse rounded-md bg-[#1E293B]/30" />
          <div className="mt-4 h-16 w-full animate-pulse rounded-lg bg-[#1E293B]/25" />
        </div>
      ))}
    </div>
  );
...
export function ExplorerMediaFeed({
  ...
  const shellClass = embedded
    ? className.trim()
    : `rounded-3xl border border-[#1E293B] bg-[#1E3A5F] p-5 shadow-lg shadow-black/20 sm:p-6 ${className}`.trim();
  ...
  return (
    ...
    >
      ...
      <header className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        {!loading && error ? (
          <button
            type="button"
            onClick={() => void load()}
            className="inline-flex items-center justify-center gap-2 self-start rounded-xl border border-[#1E293B] bg-[#1E293B] px-4 py-2.5 text-sm font-semibold text-white transition hover:border-primary/50 hover:bg-[#1E293B] motion-reduce:transition-none"
          >
            Try again
          </button>
        ) : null}
      ...
      {loading ? (
        <div className={hideNews ? "" : "space-y-10"}>
          <div>
            {!embedded ? (
              <div className="mb-4 flex items-center gap-2">
                <div className="h-9 w-9 animate-pulse rounded-xl bg-[#1E293B]/50" />
                <div className="h-4 w-32 animate-pulse rounded bg-[#1E293B]/40" />
              </div>
            ) : null}
            <ShortsRowSkeleton />
          </div>
          {hideNews ? null : (
            <div>
              <div className="mb-4 flex items-center gap-2">
                <div className="h-9 w-9 animate-pulse rounded-xl bg-[#1E293B]/50" />
                <div className="h-4 w-40 animate-pulse rounded bg-[#1E293B]/40" />
              </div>
              <NewsGridSkeleton />
            </div>
          )}
        ...
        <div
          role="alert"
          className="rounded-2xl border border-red-400 bg-[#1E293B] px-4 py-5 text-sm text-red-300"
        >
        ...
        <>
          ...
          <div className={embedded ? "relative mb-6" : "relative mb-10"}>
            {!embedded ? (
              <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
                <div className="flex items-center gap-2.5">
                  <span
                    className="flex h-9 w-9 items-center justify-center rounded-xl border border-[#1E293B] bg-[#1E293B] shadow-inner shadow-black/30 ring-1 ring-[#1E293B]/80"
                    aria-hidden
                  >
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#1E293B] bg-[#1E293B] px-3 py-2 text-xs font-semibold text-white hover:border-primary/50 hover:bg-[#1E293B]"
                  >
                  ...
                  <div className="hidden items-center gap-1 sm:flex">
                    <button
                      ...
                      className="rounded-lg border border-[#1E293B] bg-[#1E293B] p-2 text-gray-300 transition hover:border-primary/45 hover:bg-[#1E293B] hover:text-white motion-reduce:transition-none"
                    >
            ...
            {trendingShorts.length === 0 ? (
              <p className="rounded-2xl border border-dashed border-[#1E293B] bg-[#1E293B] px-4 py-10 text-center text-sm text-gray-300">
                No shorts yet for this city — check back after the cache refreshes.
              </p>
            ) : (
              <div
                ref={scrollRef}
                className="flex snap-x snap-mandatory gap-4 overflow-x-auto pb-2 pl-1 pr-1 pt-1 scrollbar-thin scrollbar-track-[#1E3A5F] scrollbar-thumb-[#1E293B] [scrollbar-color:#1E293B_#1E3A5F] [scrollbar-width:thin] [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-[#1E293B] [&::-webkit-scrollbar-track]:bg-[#1E3A5F]"
              >
                {trendingShorts.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <div className="relative h-[280px] w-[158px] overflow-hidden rounded-2xl border border-[#1E293B] bg-[#1E293B] shadow-md shadow-black/30 transition duration-500 ease-out will-change-transform hover:-translate-y-1 hover:border-primary/50 hover:shadow-lg motion-reduce:transition-none motion-reduce:hover:transform-none sm:h-[320px] sm:w-[180px]">
                          ...
                          >
                            <span className="mb-3 flex h-11 w-11 items-center justify-center self-center rounded-full bg-white/95 text-black shadow-lg ring-2 ring-primary/60 transition duration-300 group-hover:scale-110 group-hover:ring-primary motion-reduce:transition-none motion-reduce:group-hover:scale-100">
            ...
            {hideGrid && (
              <div className="mt-4 flex justify-center">
                <Link
                  href={`/explore/shorts?city=${encodeURIComponent(city)}`}
                  className="inline-flex items-center justify-center gap-2 rounded-xl border border-[#1E293B] bg-[#1E293B] px-5 py-2.5 text-sm font-semibold text-white hover:border-primary/50"
                >
                  Show more
                </Link>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
                {recentShorts.map((s) => {
                  return (
                    ...
                    >
                      <div className="relative aspect-[9/16] overflow-hidden rounded-2xl border border-[#1E293B] bg-[#1E293B] shadow-md shadow-black/30 transition duration-500 ease-out will-change-transform hover:-translate-y-1 hover:border-primary/50 hover:shadow-lg motion-reduce:transition-none motion-reduce:hover:transform-none">
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img
                            src={s.thumbnailUrl}
                            alt=""
                          ...
                          <button
                            ...
                            aria-label={`Play short: ${s.title}`}
                          >
                            <span className="mb-2 flex h-8 w-8 items-center justify-center self-center rounded-full bg-white/95 text-black shadow-lg ring-2 ring-primary/60 transition duration-300 group-hover:scale-110 group-hover:ring-primary motion-reduce:transition-none motion-reduce:group-hover:scale-100">
                ...
                <div className="columns-1 gap-4 space-y-4 sm:columns-2 xl:columns-3">
                  {visibleNews.map((article, index) => (
                    <button
                      ...
                      className="group block w-full break-inside-avoid rounded-2xl border border-[#1E293B] bg-[#1E293B] text-left transition duration-300 hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg hover:shadow-black/25 motion-reduce:transition-none motion-reduce:hover:translate-y-0"
                    >
                      <div className="rounded-[0.9rem] bg-[#1E293B] p-4 sm:p-5">
                        {article.domain ? (
                          <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-gray-400">
                            {article.domain}
                          </p>
                        ) : null}
                        <h4 className="text-[15px] font-bold leading-snug text-white transition duration-300 group-hover:text-primary motion-reduce:transition-none sm:text-base">
                          {article.title}
                        </h4>
                        <p className="mt-2 line-clamp-4 text-sm leading-relaxed text-gray-300">
                          {article.snippet}
                        </p>
                        <span className="mt-4 inline-flex items-center gap-1 text-xs font-semibold text-gray-300 transition group-hover:gap-2 group-hover:text-primary">
                          Read article
                  ...
                  <>
                    ...
                    <div className="mt-6 flex justify-center">
                      <button
                        ...
                        className="rounded-xl border border-[#1E293B] bg-[#1E293B] px-5 py-2.5 text-sm font-semibold text-white hover:border-primary/50"
                      >
