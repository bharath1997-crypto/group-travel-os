...
import { REPORT_CONFIG, minutesAgo, type RoadReport } from "@/lib/live/types";
...
function minutesUntilExpiry(iso: string): string {
...
export function RouteChatSheet({ report, onClose, onToast }: RouteChatSheetProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      sessionMessageIds.current.add(created.message_id);
      setMessages((prev) =>
        prev
  ...
  const handleFlag = async (messageId: string) => {
    try {
      await apiFetch(`/live/reports/${report.id}/chat/${messageId}/flag`, {
        method: "POST",
      });
      ...
      <div className="relative z-10 flex h-[60dvh] w-full max-w-lg flex-col rounded-t-3xl bg-white shadow-2xl">
        <div className="flex items-start justify-between border-b border-stone-200 px-4 py-3">
          <div>
            ...
            <p className="mt-1 text-xs text-stone-500">
              Anonymous chat · expires in {minutesUntilExpiry(report.expires_at)}
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
          ...
          {!loading && messages.length === 0 ? (
            <p className="py-8 text-center text-sm text-stone-500">
              No messages yet. Be the first to report conditions.
            </p>
          ) : null}

          {messages.map((msg) => {
            const isYou =
              sessionMessageIds.current.has(msg.id) || msg.sender_label === "You";
            return (
        ...
        <div className="border-t border-stone-200 px-4 py-3">
          ...
          >
            <div className="min-w-0 flex-1">
              <input
                ...
                placeholder="Share road conditions..."
          ...
          <p className="mt-2 text-center text-[11px] text-stone-400">
            Text only · no photos · anonymous · auto-deletes when report expires
          </p>
        </div>
      </div>
    </div>
