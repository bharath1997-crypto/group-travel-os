...
function ExploreCard({
  ...
  const gradientClass = "from-purple-400 to-fuchsia-600";
  ...
  const filteredItems = useMemo(() => {
    ...
    if (selectedDate) {
      result = result.filter((ev) => {
        if (!ev.date && !ev.start_date) return true; // gaming lounges are open daily
  ...
  return (
    <div className="p-6 bg-white min-h-screen">
      ...
      <div className="space-y-2">
        {renderRow("Trending Gaming", "Most popular right now")}
