...
function ExploreCard({
  ...
  const gradientClass = "from-violet-400 to-purple-600";
  ...
  const filteredItems = useMemo(() => {
    ...
    if (selectedDate) {
      result = result.filter((ev) => {
        if (!ev.date && !ev.start_date) return true; // shopping areas are open daily
  ...
  return (
    <div className="p-6 bg-white min-h-screen">
      ...
      <div className="space-y-2">
        {renderRow("Trending Shopping", "Most popular right now")}
