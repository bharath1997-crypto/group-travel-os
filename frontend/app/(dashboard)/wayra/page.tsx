...
export default function WayraPersonalPage() {
  ...
  useEffect(() => {
    void (async () => {
      ...
      let trips: WayraTripHint[] = [];
      try {
        const meRes = await apiFetchWithStatus<{ full_name?: string | null }>(
          "/auth/me",
          {},
          8000,
        );
        if (meRes.status >= 200 && meRes.status < 300) {
          fullName = meRes.data?.full_name ?? null;
        }
      } catch {
        /* guest */
      }
      try {
        const ctx = await apiFetch<WayraContext>("/wayra/context");
        fullName = ctx.full_name ?? fullName;
        trips = (ctx.trips ?? []).map((trip) => ({
          title: trip.title,
      ...
      setMessages([
        {
          id: "welcome",
          sender: "wayra",
          text: buildWayraSessionGreeting({ fullName, trips }),
          timestamp: new Date(),
        },
      ]);
  ...
  return (
    <div className="w-full bg-app py-6 px-4 md:px-8 font-sans">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-5 rounded-2xl shadow-sm border border-slate-100">
          <div className="flex items-center gap-3">
            <div className="h-12 w-12 rounded-2xl bg-primary flex items-center justify-center text-white shadow-md shadow-teal-700/20">
          ...
          <div className="lg:col-span-2 bg-white rounded-2xl border border-slate-100 shadow-sm flex flex-col h-[650px] overflow-hidden">
                  ...
                  <div key={m.id} className={`flex ${isUser ? "justify-end" : "justify-start"}`}>
                    <div className={`flex gap-3 max-w-[85%] ${isUser ? "flex-row-reverse" : "flex-row"}`}>
                      ...
                      <div className={`h-8 w-8 rounded-full flex items-center justify-center shrink-0 shadow-sm ${
                        isUser ? "bg-slate-100 text-slate-600" : "bg-primary text-white"
                      }`}>
                      ...
                      <div className="space-y-1">
                        ...
                        <p className={`text-[9px] text-slate-400 font-semibold px-1 ${isUser ? "text-right" : "text-left"}`}>
                          {formatWayraMessageTime(m.timestamp.getTime())}
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            <form onSubmit={handleSend} className="p-4 border-t border-slate-100 bg-white flex flex-col gap-2 shrink-0">
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    navigator.geolocation.getCurrentPosition(
                      ...
                      (err) => {
                        console.error(geolocationErrorMessage(err));
                      },
                      { enableHighAccuracy: true, timeout: 12000 },
                    );
                  }}
                  className="shrink-0 rounded-xl border border-slate-200 p-3 text-slate-600 hover:border-primary/30 hover:text-primary"
                  title="Attach my location"
                  aria-label="Attach my location"
                >
              ...
              <input
                ...
                className="flex-1 text-[13px] border border-slate-200 hover:border-slate-300 focus:border-primary px-4 py-3 rounded-xl outline-none transition-all font-medium text-slate-800"
              />
              <button
                type="submit"
