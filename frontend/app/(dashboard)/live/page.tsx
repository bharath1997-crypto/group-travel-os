...
export default function LivePage() {
  ...
  const [checklistReadiness, setChecklistReadiness] = useState<{ user_id: string; is_accepted: boolean }[]>([]);
    ...
    (async () => {
      try {
        const rows = await liveFetch<
          { user_id: string; full_name: string | null; avatar_url: string | null; is_accepted?: boolean }[]
        >(`/live/sessions/${session.id}/checklist`);
        if (!cancelled) {
          const m: typeof profiles = {};
          const readiness: typeof checklistReadiness = [];
          rows.forEach((r) => {
            m[r.user_id] = { full_name: r.full_name, avatar_url: r.avatar_url ?? null };
            readiness.push({ user_id: r.user_id, is_accepted: !!r.is_accepted });
          });
          setProfiles(m);
  ...
  useEffect(() => {
    if (!fb.db || !session?.trip_id) return undefined;
    const checklistRef = ref(fb.db, `trips/${session.trip_id}/live_session/checklist`);
    const off = onValue(checklistRef, (snap) => {
  ...
  const membersWithEta = useMemo(() => {
    return stripMembers.map((m) => {
      ...
      const checklistItem = checklistReadiness.find((item) => item.user_id === m.user_id);
      let etaStr = "";
      let distStr = "";
      let isStale = false;
      ...
      return {
        ...
        distance: distStr,
        isStale,
        is_accepted: checklistItem?.is_accepted || false,
      };
    });
  }, [stripMembers, membersLocs, meetPoint, groupCentroid, checklistReadiness]);
      ...
      {viewingSession && (
        <div className="fixed inset-0 z-[200] h-[100dvh] w-full overflow-hidden select-none">
          ...
          <div className="absolute inset-0 z-0">
            ...
            {/* Floating Checklist Toggle Button — top left */}
            {viewingActive && !isSoloMode && (
              <button
                type="button"
                onClick={() => setShowChecklistFloating((v) => !v)}
                className="absolute top-20 left-4 z-[4000] flex h-10 w-10 items-center justify-center rounded-full bg-slate-900/90 border border-slate-800 text-slate-350 hover:text-white shadow-lg backdrop-blur-md transition-all cursor-pointer"
            ...
            {/* Floating Checklist Panel — top left */}
            {viewingActive && !isSoloMode && showChecklistFloating && (
              <div className="absolute top-32 left-4 z-[4000] bg-slate-950/95 border border-slate-800 rounded-2xl shadow-xl max-w-[280px] w-full p-3.5 backdrop-blur-md text-slate-200">
                ...
                <div className="space-y-2">
                  ...
                  {!isUserAccepted ? (
                    <button
                      ...
                      className="w-full mt-2.5 py-2 bg-[#0F766E] hover:bg-[#0D635C] disabled:bg-slate-850 disabled:text-slate-500 text-white rounded-xl text-xs font-bold transition flex items-center justify-center gap-1.5"
                    >
