    ...
    for i in range(0, len(pois), batch_size):
        ...
        for poi in batch:
            ...
            try:
                # Check if exists
                cur.execute(
                    "SELECT id FROM explore_contents WHERE event_id = %s",
                    (event_id,)
                )
                ...
                if existing:
                    cur.execute("""
                        UPDATE explore_contents
                        SET title=%s, venue_lat=%s, venue_lon=%s,
                            city=%s, state=%s, source=%s, fetched_at=%s
                        WHERE event_id=%s
                    """, (
                        poi['name'], poi['lat'], poi['lon'],
                        poi['city'], poi['state'],
                        'openstreetmap', datetime.utcnow(),
                        event_id
                    ))
                    updated += 1
                else:
                    cur.execute("""
                        ...
                        (event_id, title, category, content_type,
                         venue_name, venue_lat, venue_lon,
                         city, state, source, fetched_at)
                ...
                logger.warning(f"Error inserting {poi['name']}: {e}")
                conn.rollback()
                continue

        conn.commit()
        logger.info(f"Progress: {min(i+batch_size, len(pois))}/{len(pois)} processed")
